...
>[!IMPORTANT]
>The setup relies on [UKI](https://wiki.archlinux.org/title/Unified_kernel_image) and built-in EFI bootloader to boot the OS. mkinitcpio uses [cmdline](https://github.com/izmyname/archlinux-backup/tree/main/etc/cmdline.d) and [kernel preset](https://github.com/izmyname/archlinux-backup/blob/main/etc/mkinitcpio.d/linux-zen.preset) to build the kernel image. The cmdline [cryptdevice](https://github.com/izmyname/archlinux-backup/blob/main/etc/cmdline.d/10-crypt.conf) _(if encryption is used)_ and [root](https://github.com/izmyname/archlinux-backup/blob/main/etc/cmdline.d/20-root.conf) must be edited on every new installation.
